...
const items = [
  {
    icon: Eye,
    title: '只读，永远不碰你的币',
    desc: '粘贴地址 ≠ 授权交易。我们只读公开链上数据，和你在区块链浏览器上查地址一样',
  },
  {
    icon: Shield,
    title: '零私钥，零风险',
  ...
  return (
    <section ref={ref} className="w-full max-w-4xl mx-auto px-5 py-20 relative z-10">
      <h2 className={`text-center font-display font-extrabold text-3xl sm:text-4xl mb-2 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        <span className="bg-clip-text text-transparent bg-gradient-to-b from-foreground to-foreground/60">安全与信任</span>
      </h2>
        ...
        {items.map((item, i) => (
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
              <item.icon size={18} className="text-primary/80" />
            </div>
            <div>
